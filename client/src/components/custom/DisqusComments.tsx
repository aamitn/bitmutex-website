...
const DisqusComments: React.FC<DisqusCommentsProps> = ({ post, currentUser }) => {
  ...
  // Stable helper — defined before useMemo so it's in scope
  const getDisqusSSOData = useCallback((user: User) => ({
    name: user.username,
    id: user.id,
    email: user.email,
    avatar: user.avatar,
  ...
  // Memoized config — never partially null, stable reference across renders
  const disqusConfig = useMemo(() => ({
    url: `${process.env.NEXT_PUBLIC_BASE_URL ?? 'https://bitmutex.com'}/blog/${post.slug}`,
    identifier: post.slug,
    title: post.title,
    sso: currentUser ? getDisqusSSOData(currentUser) : undefined,
    language: 'en_US',
  }), [post.slug, post.title, currentUser, getDisqusSSOData]);
  ...
  useEffect(() => {
    // Mark as mounted first — prevents Disqus from touching the DOM before hydration
    setMounted(true);

    const timer = setTimeout(() => {
      setIsLoading(false);
      // Replace with real API call if you have a comment count endpoint
  ...
  // Don't render any Disqus-related JSX until the client is fully hydrated.
  // This is the core fix — prevents "can't convert null to object" on remount.
        ...
        <Card className="relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-2xl border-0 ring-1 ring-gray-200/50 dark:ring-gray-700/50 rounded-2xl overflow-hidden">
          ...
          <CardHeader className="pb-6 pt-8">
            ...
            <div className="flex items-center gap-2 mt-4 text-sm text-gray-600 dark:text-gray-400">
              ...
              <span>Join the conversation about &ldquo;{post.title}&rdquo;</span>
            </div>
          </CardHeader>
          ...
          <CardContent className="px-8 pb-8">
            {isLoading ? (
              <div className="space-y-6">
                {/* Loading skeletons */}
                {[{ w: 'w-24', h: 'h-16' }, { w: 'w-32', h: 'h-12' }, { w: 'w-28', h: 'h-20' }].map(
                  ({ w, h }, i) => (
              ...
              <div className="relative">
                ...
                {/* Disqus embed — only rendered after mount + loading done */}
